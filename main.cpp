...
void rawatPasien(Dokter *dokter, int *hargaInap, int *hargaDokter, int *hargaObat, int *totalHarga, bool inap){
    ...
    if(inap){
        ...
        else if (kamar == "2") *hargaInap = 500000 * lamaInap;
        else goto pilihKamar;
    }
    ...
    const int index = stoi(pilihDokter) - 1;
    *hargaDokter = dokter[index].hargaJasa;


    pilihPenyakit: 
        tampilkanPenyakit(dokter, index);
        cout << "Pilih [1 - 2] = ";
        cin >> penyakit;
        cout << endl;
    ...
    const int index2 = stoi(penyakit) - 1;
    *hargaObat = dokter[index].daftarPenyakit[index2] -> totalHargaObat;
    tampilkanObat(dokter, index, index2);
}
